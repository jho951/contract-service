...
4. `ADMIN` 라우트면 `POST /permissions/internal/admin/verify`로 Authz 판정을 수행한다.
5. 검증 성공 결과를 L1/L2에 다시 저장한다.

## Authz Cache
- 위치: Redis
- 역할: `ADMIN` 라우트의 허용/거부 결과를 짧은 TTL로 보관
- 활성화 env: `GATEWAY_AUTHZ_CACHE_ENABLED`
- TTL env: `GATEWAY_AUTHZ_CACHE_TTL_SECONDS`
- 프리픽스 env: `GATEWAY_AUTHZ_CACHE_PREFIX`
- 기본 프리픽스: `gateway:admin-authz:`
- 비고: Authz 오류나 타임아웃은 캐시로 완화하지 않고 fail-closed를 유지한다.

## Redis Contract
- 중앙 Redis 자체 계약은 [Redis Contract](../redis-service/README.md)를 따른다.
- Gateway는 `gateway:session:`과 `gateway:admin-authz:` prefix만 사용한다.
