...
- `user-service` 현재 구현은 `platform-runtime-bom 4.0.0`, `platform-governance-bom 4.0.0`, `platform-security-bom 4.0.0`을 함께 사용한다.
- 런타임 모듈은 `platform-governance-starter`, `platform-security-starter`, `platform-security-governance-bridge`다.
- `UserPlatformRuntimeConfiguration`이 `GovernanceAuditSink`, `JwtDecoder`, prod Redis 기반 `PlatformRateLimitPort`를 제공한다.
...
- mainline compile contract는 old `AuditSink`, raw `RateLimiter`, ratelimit bridge starter를 기본 경로로 두지 않는다.
