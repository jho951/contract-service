...
- `auth-service` 현재 구현은 `platform-runtime-bom 4.0.0`, `platform-governance-bom 4.0.0`, `platform-security-bom 4.0.0`을 함께 사용한다.
- 런타임 모듈은 `platform-governance-starter`, `platform-security-starter`, `platform-security-governance-bridge`다. auth/rate-limit bridge starter는 mainline에 두지 않는다.
- `AuthPlatformIssuerAdaptersConfiguration`이 `PlatformTokenIssuerPort`, `PlatformSessionIssuerPort`, `PlatformSessionSupportFactory`를 직접 제공한다.
- `AuthPlatformOperationalConfiguration`은 `GovernanceAuditSink`와 prod Redis 기반 `PlatformRateLimitPort`를 제공한다.
...
- mainline compile contract는 raw `TokenService`, `SessionStore`, `RateLimiter`, old `AuditSink`를 기본 surface로 두지 않는다.
