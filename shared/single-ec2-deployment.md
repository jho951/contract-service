...
이 문서는 현재 AWS Free Tier 기준의 실제 배포 모드를 정의한다.
...
- `editor-service`
- `redis-service`
- `monitoring-service` 선택

## 1. 목적

이 모드는 다음 조건에서 사용한다.

- AWS Free Tier 범위를 최대한 벗어나지 않아야 한다.
- NAT Gateway, ALB, Route53 private hosted zone, Fargate 비용을 피해야 한다.
...
5. Monitoring은 리소스가 충분할 때만 같은 host에 최소 구성으로 올린다.
...
- Free Tier 범위에서 시작할 때는 `t2.micro` 또는 `t3.micro`를 기본으로 본다.
- public IP 또는 Elastic IP를 하나만 붙인다.
- root volume은 최소 `20GB`를 권장한다.
...
- 단일 EC2 개발/초기 운영 모드에서는 Redis host publish보다 Docker internal access를 우선한다.
...
단일 EC2 모드에서는 monitoring을 두 단계로 나눈다.

### 최소 운영

- 각 서비스 health/readiness만 먼저 확인한다.
- Gateway 로그와 앱 로그를 우선 본다.
